...
public class SaveSystemScript : MonoBehaviour
{
	...
	public void sendRanking()
	{
		...
		GooglePlaySystem.instance.SendRanking(playerSO.knowledgePoints);
	}
	...
	private string LoadLocal()
	{
		...
		if (!File.Exists(PLAYER_PATH) || string.Equals(version, "0.0.0"))
		{
			...
			Debug.Log("Primera vez que se instal�");
		}
	...
	{
		if (contentServer) StartCoroutine(GetJsonHeroku());
		else StartCoroutine(GetJsonDEGA());
	}

	private IEnumerator GetJsonDEGA()
	{
	...
	private IEnumerator GetJsonHeroku()
	{
		string jsonRemote = LoadRemote();
		int n;
